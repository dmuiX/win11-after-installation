...
param(
    [switch]$FixWinget,      # reset winget sources
...
if (-not $selective -or $Aliases) {
    ...
    if (Get-Command git -EA 0) {
        ...
        $clinkExe = "${env:ProgramFiles(x86)}\clink\clink_x64.exe"
        if (-not (Test-Path $clinkExe)) { $clinkExe = "$env:ProgramW6432\clink\clink_x64.exe" }
...
if (-not $selective -or $Maintenance) {
    ...
    $scheduled = 0
    foreach ($t in $tasks) {
        $src = "$ScriptRoot\configs\$($t.Src)"
        if (Test-Path $src) {
            $dst = "$env:USERPROFILE\$($t.Src)"
            Copy-Item $src $dst -Force
            $tr = if ($t.Src -match '\.ps1$') { "pwsh.exe -NoProfile -ExecutionPolicy Bypass -File `"$dst`"" } else { "`"$dst`"" }
            schtasks /create /tn $t.TN /tr $tr /sc $t.Sch /st 12:00 /rl HIGHEST /f | Out-Null
            $scheduled++
        }
        else {
            Show-Error "$($t.Src) not found in configs"
        }
    }
    if ($scheduled -gt 0) { Show-OK "$scheduled maintenance task(s) scheduled." }
    else { Show-Error "No maintenance tasks created (config files missing)" }
...
if (-not $selective -or $Network) {
    ...
    if ($regVal -ne $ipv6Data) {
        ...
        Show-OK "IPv6 disabled (0xFF)."
        $restartRequired = $true
    }
    else {
        Write-Host " [OK] IPv6 already disabled." -ForegroundColor Green
    }
...
if (-not $selective -or $Network) {
    if ($restartRequired) {
        Write-Host "RESTART REQUIRED: IPv6 was disabled." -ForegroundColor Red
