...
winget list --accept-source-agreements > "%INSTALLED_CACHE%"
...
call :winget_install marha.VcXsrv
...
echo.
echo Copying 'clink_display.lua' to Clink directory...
copy /Y "%~dp0clink_display.lua" "%LOCALAPPDATA%\clink\display.lua"
...
:: Create the aliases file. The first line uses > to create a new file.
(
    echo ls=lsd -l
...
:: Set the Autorun registry key
:: This command tells cmd.exe to first inject Clink, then load the aliases using doskey.
:: Note: This assumes Clink is installed in the default location. Adjust the path if necessary.
...
:: Define the full path for the .vimrc file
set "VIMRC_FILE=%USERPROFILE%\.vimrc"

:: This block writes the entire multiline content to the file.
:: The `>` operator creates the file or overwrites it if it already exists.
:: Special characters like |, <, >, and & are escaped with a caret (^).
(
    echo " ============================================================================
...
:: Define the full path for the AutoHotkey script file on the user's Desktop
set "AHK_FILE=%USERPROFILE%\Desktop\macos-hotkeys.ahk"

:: This block writes the multiline content to the file.
:: The > operator creates the file or overwrites it if it already exists.
:: The caret character (^) is escaped by doubling it (^^) so batch treats it literally.
echo.
...
:: Set StateFlags0001 = 2 (Selected) / 0 (Not Selected)
:: User Goal: Remove everything EXCEPT Downloads and Recycle Bin
